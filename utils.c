...
makeArgs(const char * cmd, int * retArgc, const char *** retArgv)
{
	...
	*retArgv = NULL;
	...
	// Si nous sommes dans une built-in
	if (argTable[0][0] == '-' && strcmp(argTable[0], "-arith") != 0){
	  int i;
	  // Pour tous les arguments
	  for (i = 1; i < argCount; i++){
	    // Les arguments ayant moins de deux caractères ne peuvent pas être évalués
	    if (strlen(argTable[i]) < 2) continue;
	    // Si l'argument débute par $ alors, on réalise un traitement particulier
	    if (argTable[i][0] == '$'){
	      // Si l'argument commence par [ et finit par ], on évalue son corps
	      unsigned int last_char_idx = strlen(argTable[i]) - 1;
	      if (argTable[i][1] == '[' && argTable[i][last_char_idx] == ']'){
		char *b = strdup(argTable[i]);
		b[last_char_idx] = '\0';
		char *buff = eval(b+2);
		argTable[i] = buff;
	      } 
	      // Si on veut uniquement le pid :
	      else if (argTable[i][1] == '$'){
		char buff[16]; // longueur max d'un pid?
		sprintf(buff, "%d", getpid());
		argTable[i] = strdup(buff);
	      } 
	      // Sinon c'est juste une variable,
	      // on utilise quand même notre fonction
	      else {
		char *buff = eval(strdup(argTable[i]));
		argTable[i] = strdup(buff);
	      }
	    }
	  }
